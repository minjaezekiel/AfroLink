...
| **Sovereign currencies are issued on-chain, under split keys** | A central bank registers as the issuer of `sov/ke/kes` and authorises licensed intermediaries to mint, each with a finite allowance that every mint spends down. The cold authority configures and never issues; a stolen hot key costs what was left on it and no more. Burning takes money out of the sender's *own* balance — there is no way to spell "destroy that account's money" — and a declared supply cap can be tightened and never loosened. |
...
curl 'localhost:8080/v1/accounts/afri1…/history'                    # a fast hint
curl 'localhost:8080/v1/transactions/{id}'                          # proof + receipt
...
* **~232 000 hostile byte strings** across 58 decoder fixtures, asserting that
  ...
  `touched` list rather than sorting it.
