...
| **`SendRate` / `RecvRate`** — per-connection limits in bytes per second, against a real clock | **In code.** Ours is two token buckets per peer, messages and bytes, refilled by elapsed time *handed in* so the policy still reads no clock. Adopted after a limit denominated in "per tick" was loosened tenfold by an unrelated change to a loop period | [ADR-0024](adr/0024-block-sync-and-the-node-binary.md) | **In code** |
| **`signAddVote`** — a validator adds its own vote to its own vote set, and gossip is downstream of that | **In code.** Adopted after the opposite arrangement — the transport looping a node's own votes back — was found to make a consensus invariant depend on a transport being present | [ADR-0024](adr/0024-block-sync-and-the-node-binary.md) | **In code** |
| **`MaxPacketMsgPayloadSize`** — a peer can never announce more than one small packet | **Partly.** Frames are read in 16 KiB chunks so an announced length costs nothing until it arrives, which bounds the memory. The stronger form needs `PartSet` below | ADR-0024 | **Partly** |
| **`PartSet`** — a block gossiped as Merkle-ized 64 KiB parts, with validators agreeing on a `BlockID` | **Open.** The structural answer to fitting a block in a message: the largest thing a peer can ask a node to hold becomes a constant. A change to the consensus wire format, so not taken lightly | ADR-0024 | **Open** |
