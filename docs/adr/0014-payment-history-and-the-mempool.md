...
completeness proof for history, which would need the index to be part of
consensus state — a much larger change, and the right one to consider if
omission ever proves to be a problem in practice.

## Revisit if

- A node is caught omitting history entries, which is the trigger for either
  multi-node corroboration or an in-state index
